...
| **P0** | 2 | Chief of Staff Virtuel IA (COS) | **Doc + procédure** : ce dépôt ([`cos-replit-ec2-migration-2026-05-04.md`](./cos-replit-ec2-migration-2026-05-04.md)) ; **dépôt applicatif GitHub** encore **à créer ou confirmer** | Runbook : EC2 Windows / Caddy / tâches `WeAdU-COS` — [WEA-29](./WEA-29-aws-ec2-inventory.md) §3 + fichier COS ; secrets hors Repl ([WEA-15](../SECRETS_SOCLE_WEA15.md)). |
| **P1** | 3 | after-framfield-cockpit | **`JeffWeadu/after-framfield-cockpit`** (perso — repo à créer ; runbook [§3.1](./after-framfield-cockpit-replit-2026-05-05.md)) | README + redirect OAuth stable — [WEA-20](../GOOGLE_OAUTH_WEA20.md). |
...
3. **P1** : basculer **negative-search-terms-tool** et **wellbots-shopping-ads-checker** vers les dépôts connus ; **after-framfield-cockpit** → repo perso [`JeffWeadu/after-framfield-cockpit`](https://github.com/JeffWeadu/after-framfield-cockpit) (création manuelle puis import — [runbook](./after-framfield-cockpit-replit-2026-05-05.md)).
...
| **2 — Chief of Staff Virtuel IA (COS)** | **Prod** documentée sur **EC2** (pas Replit pour `leadgen.generads.com`) — voir [WEA-33](./WEA-33-replit-inventory.md) #2 et [`cos-replit-ec2-migration-2026-05-04.md`](./cos-replit-ec2-migration-2026-05-04.md). Le **Repl COS** peut encore exister en dev / archive ; **dépôt GitHub applicatif** non encore identifié sous `WeAdU-ltd`. | Fermer le Repl ou documenter qu’il est vide ; **créer** le repo applicatif + README quand le code source est basculé depuis l’hôte ou le Repl ; puis retirer cette ligne ([WEA-38](https://linear.app/weadu/issue/WEA-38/replit-fermeture-apres-bascule-complete)). |
| **3 — after-framfield-cockpit** | **Perso** — cible GitHub **`JeffWeadu/after-framfield-cockpit`** ; création du repo refusée par API depuis l’agent (création manuelle requise). OAuth workspace encore éphémère jusqu’à cutover. | Créer le repo sur github.com ; pousser le code ; stabiliser OAuth ([WEA-20](../GOOGLE_OAUTH_WEA20.md)) ; puis retirer la ligne ([WEA-38](https://linear.app/weadu/issue/WEA-38/replit-fermeture-apres-bascule-complete)). |
